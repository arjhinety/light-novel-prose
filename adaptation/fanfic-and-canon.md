...
3. **Research, then write a canon sheet for each character** (§3), with confidence markers (§4). **Record canon fanservice traits, then note how they render.** Many harem and rom-com sources run fanservice gags (a heroine scheming for physical contact, jealousy about figures, lucky-pervert accidents). Record them, because they're part of who the character is. Then write two things next to each one: how it plays **for this chapter's ages**, and which register it belongs to in [Ecchi & Fanservice](../genres/ecchi-and-fanservice.md) §3. Adult characters are unrestricted — write the trait at whatever heat the request names, including explicit. Characters under 18 get no sexual rendering at all: the scheme gets interrupted, someone blushes, the tsundere hits him, and there is no camera on a body. Age the cast up, move the beat to an adult character, or play it as comedy, and say which in the notes. If your environment can browse or read files, look up each character first (a wiki page, a character profile), and note which lookups failed. **Canon confidence gate:** if any *main* character's core trait, speech mode, or form of address is still marked unsure after research, don't guess. Ask the user one short question ("How does Shizuka talk to Rentarou: through her phone's text-to-speech, in her own voice, or both?"), or drop that character from the chapter. In the test runs, models that guessed produced a cat-girl version of a stoic character and gave a shy girl a nickname she'd never use, and both reviewers ranked canon accuracy as the worst failure.
